...
from DDPG_model import Actor, Actor_Feature, Critic, Meta_Critic, Meta_Attention
...
class DDPG(object):
    ...
    def select_action(self, state):
        ...
        if (self.args.method == 'DDPG_MAT4'):
            ...
            attention_action_Q = self.critic(state, attention_action).cpu().data.numpy()
            action_Q = self.critic(state, action).cpu().data.numpy()
    ...
    def train(self, replay_buffer, training_step, batch_size=100, discount=0.99, tau=0.005):
        ...
        if training_step % 1 == 0:
            ...
            # Meta Critic Update
            if (self.args.method == 'DDPG_MC'):
                self.hotplug.update(self.lr_actor)
                state_feature_val = self.actor_feature(state_val)
                policy_loss_val_mc = self.critic(state_val, self.actor(state_feature_val))
                ...
                policy_loss_val_mc_new = self.critic(state_val, self.actor(state_feature_val_new))
            ...
            if ((self.args.method == 'DDPG_MATT') or (self.args.method == 'DDPG_MAT4')):
                self.hotplug.update(self.lr_actor)
                actor_features = self.actor_feature(state_val)
                policy_loss = self.critic(state_val, self.actor(actor_features))
                ...
                policy_loss_ma = self.critic(state_val, attention_action)
                ...
                policy_loss_new = self.critic(state_val, self.actor(actor_features_new))
